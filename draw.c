...
void draw_velocity_vector(Circle *c) {
  ...
  // Flip Y velocity to match SDL coordinate system (positive Y = down)
...
void draw_settings_panel() {
  if (!state.settings.show_settings || !state.font) return;
  ...
  SDL_Rect panel_rect = {panel_x, panel_y, SETTINGS_PANEL_WIDTH, SETTINGS_PANEL_HEIGHT};
  ...
  // Title
  SDL_Surface *title_surface = TTF_RenderText_Solid(state.font, "Settings", white);
  if (title_surface) {
    SDL_Texture *title_texture = SDL_CreateTextureFromSurface(state.renderer, title_surface);
    if (title_texture) {
      SDL_Rect title_rect = {panel_x + 10, y_offset, title_surface->w, title_surface->h};
  ...
  if (gravity_surface) {
    SDL_Texture *gravity_texture = SDL_CreateTextureFromSurface(state.renderer, gravity_surface);
    if (gravity_texture) {
      SDL_Rect gravity_rect = {panel_x + 10, y_offset, gravity_surface->w, gravity_surface->h};
  ...
  SDL_Surface *particles_surface = TTF_RenderText_Solid(state.font, text, white);
  if (particles_surface) {
    SDL_Texture *particles_texture = SDL_CreateTextureFromSurface(state.renderer, particles_surface);
    if (particles_texture) {
      SDL_Rect particles_rect = {panel_x + 10, y_offset, particles_surface->w, particles_surface->h};
  ...
  snprintf(text, sizeof(text), "Velocity: %.0f to %.0f", state.settings.initial_velocity_min, state.settings.initial_velocity_max);
  SDL_Surface *velocity_surface = TTF_RenderText_Solid(state.font, text, white);
  if (velocity_surface) {
    SDL_Texture *velocity_texture = SDL_CreateTextureFromSurface(state.renderer, velocity_surface);
    if (velocity_texture) {
      SDL_Rect velocity_rect = {panel_x + 10, y_offset, velocity_surface->w, velocity_surface->h};
  ...
  if (status_surface) {
    SDL_Texture *status_texture = SDL_CreateTextureFromSurface(state.renderer, status_surface);
    if (status_texture) {
      SDL_Rect status_rect = {panel_x + 10, y_offset, status_surface->w, status_surface->h};
  ...
  SDL_Surface *controls_surface = TTF_RenderText_Solid(state.font, "Controls:", white);
  if (controls_surface) {
    SDL_Texture *controls_texture = SDL_CreateTextureFromSurface(state.renderer, controls_surface);
    if (controls_texture) {
      SDL_Rect controls_rect = {panel_x + 10, y_offset, controls_surface->w, controls_surface->h};
  ...
  SDL_Surface *space_surface = TTF_RenderText_Solid(state.font, "SPACE - Pause/Resume", white);
  if (space_surface) {
    SDL_Texture *space_texture = SDL_CreateTextureFromSurface(state.renderer, space_surface);
    if (space_texture) {
      SDL_Rect space_rect = {panel_x + 10, y_offset, space_surface->w, space_surface->h};
  ...
  SDL_Surface *step_surface = TTF_RenderText_Solid(state.font, "J - Step simulation", white);
  if (step_surface) {
    SDL_Texture *step_texture = SDL_CreateTextureFromSurface(state.renderer, step_surface);
    if (step_texture) {
      SDL_Rect step_rect = {panel_x + 10, y_offset, step_surface->w, step_surface->h};
  ...
  SDL_Surface *reset_surface = TTF_RenderText_Solid(state.font, "R - Reset simulation", white);
  if (reset_surface) {
    SDL_Texture *reset_texture = SDL_CreateTextureFromSurface(state.renderer, reset_surface);
    if (reset_texture) {
      SDL_Rect reset_rect = {panel_x + 10, y_offset, reset_surface->w, reset_surface->h};
  ...
  SDL_Surface *vectors_surface = TTF_RenderText_Solid(state.font, "V - Toggle vectors", white);
  if (vectors_surface) {
    SDL_Texture *vectors_texture = SDL_CreateTextureFromSurface(state.renderer, vectors_surface);
    if (vectors_texture) {
      SDL_Rect vectors_rect = {panel_x + 10, y_offset, vectors_surface->w, vectors_surface->h};
  ...
  SDL_Surface *settings_surface = TTF_RenderText_Solid(state.font, "S - Toggle settings", white);
  if (settings_surface) {
    SDL_Texture *settings_texture = SDL_CreateTextureFromSurface(state.renderer, settings_surface);
    if (settings_texture) {
      SDL_Rect settings_rect = {panel_x + 10, y_offset, settings_surface->w, settings_surface->h};
  ...
  SDL_Surface *quit_surface = TTF_RenderText_Solid(state.font, "Q - Quit", white);
  if (quit_surface) {
    SDL_Texture *quit_texture = SDL_CreateTextureFromSurface(state.renderer, quit_surface);
    if (quit_texture) {
      SDL_Rect quit_rect = {panel_x + 10, y_offset, quit_surface->w, quit_surface->h};
